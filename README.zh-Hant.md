...
| ✨ **一鍵最佳化** | 標題列一顆按鈕，把推薦大小套用到所有外接螢幕 |
| 🙋 **「我偏好」開關** | 大字／均衡／空間，連動所有推薦與大小標籤 |
| 🛟 **安全切換** | 有風險的變更（更新率、非 HiDPI、比例不同）會跳出「畫面正常嗎？」倒數，沒回應自動還原 |
| 📏 **尺寸校正** | 螢幕沒回報尺寸或回報錯誤？點一下選吋數即可 |
| 💾 **保存守護** | 偵測沒有覆寫檔支撐的 HiDPI 檔位（重開機會消失），一鍵寫入系統 |
| 🔁 **重開機後自動套用** | 重開機前選好大小，開機後自動幫你切換 |
| 🖱 **選單列快速切換** | 右鍵點圖示就能切換大小，不必打開面板 |
